...
public class Tablero {
    ...
    private boolean entraElBarco(Nave nave, int filInicial, int colInicial) {
        if (!validarCoordenadas(filInicial, colInicial)) { // chequeo las coord iniciales esten dentro del rango de la
                                                           // matriz
            System.out
                    .println("Error al colocar " + nave.getTipo() + " coordenadas iniciales fuera de rango de matriz");
            return false;
        }
    ...
    public boolean disparar(int f, int c, int danio) {
        ...
        if (celdaOcupada(f, c)) {
            ...
            if (matriz[f][c].getTipo().equals("X") || matriz[f][c].getTipo().equals("0")) {
                ...
                if (matriz[f][c].getVida() > 1) {// si el barco tiene mas de 1 de vida se le quita la vida y se marca
                                                 // con impacto "X" el lugar
                    matriz[f][c].quitarVida(danio);
                    matriz[f][c] = new Impacto();
                    System.out.println("Disparo efectivo");
                    return true;
                } else { // si el barco tiene 1 de vida se hace lo mismo pero se informa que el barco fue
                         // destruido
                    matriz[f][c].quitarVida(danio);
                    matriz[f][c] = new Impacto();

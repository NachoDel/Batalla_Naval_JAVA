...
public class Tablero {
    ...
    // retorna true si el disparo pudo hacerse, false si esa zona ya fue disparada previamente
    public boolean recibirDisparo(int f, int c, int danio) {
        ...
        if (celdaOcupada(f, c)) {
            ...
            if (matriz[f][c] instanceof Impacto || matriz[f][c] instanceof Agua ) {
                ...
                if (matriz[f][c].getVida() - danio > 0) {// si el barco aguanta el tiro sin morir se le quita la vida y se marca
                                                        // con impacto "X" el lugar
                    matriz[f][c].quitarVida(danio);
                    matriz[f][c] = new Impacto();
                    System.out.println("Disparo efectivo");
                    return true;
                } else { // si el barco no aguanta el tiro se hace lo mismo pero se informa que el barco fue
